...
inline script_wait() /* ScriptExecutionControl::wait(const int &milliseconds) */
{
	...
	timerTimeout = false; /* это означает в модели, что мы указали достаточно большое время, за которое может произойти многое */
...
proctype User() /* процесс, который моделирует возможные вызовы методов в trikScriptRunner */
{
	...
	emit(GUIThreadEvents, abortScript); /* имитируем для "разнообразия" в конкретной модели, иначе при кратчайшем пути pan -i -A эмитится второй запуск */
//	if
//	:: emit(GUIThreadEvents, runScript);
//	:: emit(GUIThreadEvents, abortScript);
//	fi;
...
//never  {    /* !<>[]p4 */ /* хотим проверить, что не будет лайвлоков */
/*T0_init:
        do
        :: (! ((p4))) -> goto accept_S9
        :: (1) -> goto T0_init
