...
inline joinThread(idT) /* Threading::joinThread(const QString &threadId) */
{
	...
	do // разобраться, почему есть прогресс
	:: /* длинное условие, которое, возможно, потребуется явно прописать тут, для более адекватной модели - "пока или не закончил или не начал работать" */
		...
		if 
		:: mResetStarted -> goto joinThread_return; // можно в будущем проверить, что не будет тут бесконечного цикла, даже есть был threading_reset
		:: else -> skip;
		fi;
		lock(mThreadsMutex, _mThreadsMutex, _pid);
...
inline threading_reset() /* Threading::reset() */
{
	...
	mResetStarted_enter: mResetStarted = true;
	unlock(mResetMutex, _mResetMutex, _pid);
	...
	mResetStarted_exit: mResetStarted = false;
...
T0_init:
        ...
        :: atomic { (! ((p2)) && ! ((p3)) && (p1)) -> assert(!(! ((p2)) && ! ((p3)) && (p1))) }
        :: (1) -> goto T0_init
        od;
accept_S4:
        do
        :: (! ((p2))) -> goto accept_S4
        :: atomic { (! ((p2)) && ! ((p3))) -> assert(!(! ((p2)) && ! ((p3)))) }
        od;
accept_all:
        skip
}
