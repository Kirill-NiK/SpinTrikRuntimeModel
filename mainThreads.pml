...
mtype {FailedToOpenFileException}; /* бросаемые исключения */
...
inline startThread() /* Threading::startThread(...) --- в этом методе не важно, что за имя треда и функции нам передают, поэтому выбираем недетерминированно */
{
	...
	short myThread;
	atomic {
		threadId != -1; /* искусственно запрещаем создавать больше S тредов */ // как вариант, можно выходить в цикл обработки ивентов при -1
		myThread = threadId;
		mThreads[myThread] = 1;
	...
	:: engineThreadEvents[id] ? signal ->
		...
		:: signal == start -> /* ScriptThread::run() */
			...
			:: (!abortEvaluationInvoked[id]) -> /* если не был вызван аборт исполнения скрипта */
				:: 
					evalSystemJs();
					copyRecursivelyTo: skip; /* рекурсивное копирование */ // нужно быть уверенным, что оно не будет бесконечным
					evalSystemJs();
			...
			:: LOG("Uncaught exception at line ...");
			::
				if
				:: mInEventDrivenMode -> 
			...
			atomic {
				...
				// это ведь плохо, если перед удалением у нас висят какие-то ивента в ивентлупе?...
				// но, возможно, нет. так как они мб не обрабатываются. тогда нужно тут очищать очередь.
				mThreadCount--;
			};
...
proctype GUIThread()
{
	...
	:: GUIThreadEvents ? signal -> /* расписываем ВСЕ возможные сигналы для каждого потока */
		...
		:: signal == completed ->
			if
			:: hideRunningWidgetSignal: skip;
			:: sendMessages: skip; showErrorSignal: skip; /* по всем открытым соединениям вещается об ошибке */
			fi;
			brick_reset();
		:: signal == completed ->
...
proctype ExceptionHandler() /* процесс, который моделирует обработку исключенений */
