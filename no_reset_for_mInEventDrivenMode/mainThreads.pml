...
bool RunningWidget = false; /* флаш для модели, показывающий, что RunnungWidget запущен */
...
inline abort(thrId) /* ScriptThread::abort() для конкретного нашего потока: thread->abort(); */
{
	...
	endUselessCalls__: emit(engineThreadEvents[thrId], stopRunning);
}
...
{
	timerTimeout = true; // поправить! вышла ошибка, которая сообщает, что кто-то будет долго ждать выхода ...
//	if // < 0, 0, >0, <2^32
//	:: run Timer();
...
inline threading_reset() /* Threading::reset() */
{
	...
	mThreadCount == 0; /* waitForAll(); */ // тут был найдет лайвлок, временный фикс - добавляем метку конечного состояния.?????
...
proctype GUIThread()
{
	...
	:: GUIThreadEvents ? signal -> /* расписываем ВСЕ возможные сигналы для _КАЖДОГО_ потока */
		...
		:: signal == abortScript -> // а точно ли в этом потоке этот сигнал испускается?
			stopScript();
			LOG("Stopping robot");
			brick_reset();
			//assert(mResetMutex);
			//assert(mThreadsMutex);
			//assert(!mInEventDrivenMode);
	    fi;
	od;
}
...
proctype User() /* процесс, который моделирует возможные вызовы методов в trikScriptRunner */
{
	...
	RunningWidget ->
	if
	:: !RunningWidget -> emit(GUIThreadEvents, runScript);
	:: skip;//emit(GUIThreadEvents, abortScript);
	fi;
